const MESSAGES = {
  DUPLICATE_REQUEST: "연속적인 로그인이 식별되었습니다.",
  POPUP_BLOCKED: "팝업이 차단되어있습니다.",
  POPUP_CLOSED: "로그인 진행중 팝업이 종료되었습니다.",
  UNEXPECTED_ERROR: "예상치 못한 오류가 발생하였습니다.",
  EXPIRED_TOKEN: "토큰이 만료되었습니다.",
  FAILURE_LOGOUT: "로그아웃을 실패하였습니다.",
  INCLUDING_SPECIAL_CHARACTER: "특수문자를 포함할 수 없습니다.",
  INCLUDING_SPACE: "공백을 포함할 수 없습니다.",
  INSUFFICIENT_LENGTH: "두 글자 이상 입력해 주세요.",
  EXCESSIVE_LENGTH: "최대 10자리까지 입력 가능합니다.",
  EXCESSIVE_HASHTAG_NUMBER: "해시태그 개수는 5개를 초과할 수 없습니다.",
  EXCESSIVE_HASHTAG_LENGTH: "각 해시태그는 최대 15자리까지 입력 가능합니다.",
  UNSUITABLE_CASE: "모든 해시태그는 PascalCase로 입력되어야 합니다.",
  EXCESSIVE_IMAGE_SIZE: "첨부파일 사이즈는 1MB 이내로 등록 가능합니다.",
  FAILED_UPLOAD_IMAGE: "이미지를 업로드하실 수 없습니다.",
  OK: "ok",
};

export default MESSAGES;
